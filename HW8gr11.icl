...
sum_areas ls = toInt(sum [(toReal x.width) * (toReal x.height) * (0.5)\\x<-ls|x.pos.x >=0 && x.pos.y >=0])
...
//Start = [y\\x<-:"Give me liberty or give me death" & y <-[0..]| x == ' ']
genAux :: Char -> Char
genAux x
| x == 'i' || x == 'I' = '1'
| x == 'o' || x == 'O' = '0'
...
generate str 
| size str == 0 = ""
= toString (map (genAux)fls)
where ps = [y\\x<-:str & y <-[0..]| x == ' ']
	  ls = [(str.[x+1])\\x<-ps]
	  fls =insertAt 0 str.[0] ls
